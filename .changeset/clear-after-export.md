...
**The confirmation gained the disposal, scoped to the exported revisions.** The export captures the artifact's `(id, updatedAt)` pairs, and `Clear comments` deletes only comments still at one of those revisions. A note added while the panel was open, or an exported note edited since, is feedback the file does not hold — so the clear leaves it alone, and "The exported file is unaffected" is true by construction. When none of the batch remains, the control retires instead of arming a "Clear 0 comments?".
...
The panel's status line is now an `aria-live="polite"` region with a write generation, so a slow clipboard retry can neither go unannounced (the old `Copy failed` rewrite was silent) nor overwrite the armed warning at the decision moment. The quiet-control opacity and the stock `--pf-danger` fallback (now `light-dark(#dc2626,#f87171)`) were retuned so both the resting and armed states clear WCAG AA 4.5:1 on both schemes, and panel buttons get a 44px floor under 640px per the spec's mobile hit-target rule.

Budget: local figures land at 22.16 KB gz (IIFE) and 22.00 KB gz (ESM); the ceilings move to 22.30/22.15 KB as a deliberate trade, set generously and re-ratcheted to the figure CI reports once the release lands.
