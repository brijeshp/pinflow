...
- **Size budgets are hard ceilings**, enforced by `pnpm size` (`size-limit` in `package.json` is the source of truth). Budgets only ever ratchet down between features; a feature may move a ceiling up only as a deliberate, changeset-documented trade approved by the repo owner, immediately re-ratcheted razor-thin over actuals (policy details: `docs/wiki/build-and-release.md`). Check the budget impact of every core change.
  - **A local `pnpm size` is not the gate — it reads roughly 20 B under linux CI.** Set ceilings from the number CI reports, never from a local macOS build, and leave enough margin that the difference cannot decide it. 0.6.0 failed CI by **one byte** (17601 against a 17600 ceiling) because the ceiling was set to 17.6 from a local 17.58: a 20 B gap on a 20 B margin. "Razor-thin over actuals" means thin over the **CI** actual; ~50 B is thin, 20 B is a coin flip. If you have only a local number, raise the ceiling, push, read the CI figure, then re-ratchet to it.
