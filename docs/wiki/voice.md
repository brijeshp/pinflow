...
Voice loads on demand via `src/core/voice-loader.ts` `loadVoice()`, a dynamic import of the `pinflowjs/voice` specifier marked `external` in `tsup.config.ts` (both ESM and IIFE configs). Consequences:

- Core bundles (`dist/index.js`, `dist/pinflow.iife.js`) never include voice code; the import stays a runtime reference.
- `tsup.config.ts` sets `external: ['pinflowjs/voice']` so the seam holds across all output formats.
...
- On the ESM path, the consumer's bundler resolves `pinflowjs/voice` via the package exports map. On the IIFE/CDN path the import fails gracefully and the caller degrades to text; a dedicated CDN voice loader is deferred.
...
- ❌ Import voice from core: `import ... from 'pinflowjs/voice'` anywhere in `src/core/**` other than the dynamic import inside `voice-loader.ts`.
