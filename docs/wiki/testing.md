...
- **Aliases** (`vitest.config.ts`): `pinflowjs/voice` and bare `pinflow` resolve to `src/` sources so lazy voice stays lazy and wrappers exercise core internals without a build step.
