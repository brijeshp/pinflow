...
| Entry         | Budget (gz)                                     |
| ------------- | ----------------------------------------------- |
| core IIFE     | 22.30 KB (generous, pending post-merge ratchet) |
| core ESM      | 22.15 KB (generous, pending post-merge ratchet) |
| voice ESM     | 4.45 KB                                         |
| react wrapper | 0.47 KB                                         |
| vue wrapper   | 0.61 KB                                         |

The two core ceilings are mid-trade for the 0.10.0 export-confirmation clear (see that changeset): set generously pre-push, to be re-ratcheted to the CI figure + ~50 B once the release lands, per the policy below.
