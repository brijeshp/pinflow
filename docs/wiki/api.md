...
- Recent pre-1.0 breaks (see `.changeset/` and `CHANGELOG.md`): removed `position`/`hidden` config; explicit Save + Escape-to-discard popup flow; stealth identity deferred to first gesture; Vue `onSubmit` → `submitHandler`; host-defined `routeKey` + `refreshRoute()`; 0.5.0 removed the reviewer menu panel (control pill is a pure arm/disarm toggle; Clear all folded into Export & clear; `onSubmit`'s button moved to the export sheet).
