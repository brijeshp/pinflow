...
| `submitTo`      | `{ email: string; subject?: string }`                           | free-tier hand-off: the post-export confirmation gains an "Email it to the builder" `mailto:` button; subject defaults to `Feedback: <project>`. The body copy adapts to what actually succeeded — a download cannot be observed (detached `a.click()` returns `void`, and it no-ops in some in-app webviews), so the panel names the clipboard when that worked and otherwise tells the reviewer to attach the downloaded file rather than claiming either landed |
...
| `activation`    | `{ mode?: 'toggle' \| 'stealth' \| 'both' }`                    | default `both`: dock arm segment + Alt gestures (⌥+click = point on release, ⌥+drag = area, both ⌥ on Mac) / 500 ms long-press; `toggle` = arm segment only (no gestures); `stealth` = gestures only (no arm segment)                                                                                                                                                                                                                                              |
