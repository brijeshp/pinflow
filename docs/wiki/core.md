...
Area footprints: every visible area comment renders a `.area` marching-ants region (four 1px `repeating-linear-gradient` edges in `currentColor` + faint wash, `pointer-events:none`, ants frozen under `prefers-reduced-motion`) beside its pin — created in `_renderPins`, repositioned in `_repositionPins` via `_placeArea()` (anchored element's live rect × `areaPercent`, same cached-anchor path as pins), muted via `[data-status]`, hidden with orphaned pins, shown in builder mode.
...
**Shadow DOM structure** (host element carries `data-pinflow-root`): `<style>`, `.root` (fixed full-viewport, pointer-events:none), the `.dock` (arm segment + count chip), numbered `.pin` badges (position:fixed), `.panel`, `.input`, builder `.drawer`, and a voice-dot mount.
