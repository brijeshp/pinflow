...
A minted handle is a storage key, never a display name. `isAnonymous()` gates it out of every artifact: `_displayName()` in the annotator returns `''` for it, so `exportReviewer()` drops both the `— from` in the heading and the `Reviewer:` line, and `exportFilename()` drops the who segment. (`exportFilename` distinguishes three cases: `null` is the builder aggregate, `''` is an unnamed reviewer, a string is a name.)

The export sheet carries the optional name field (`input.name`, prefilled from `_displayName()`, Enter exports). `_adoptTypedName()` runs before the artifact is built and calls `renameReviewer()` in `storage.ts`, because the corpus key embeds the reviewer — naming yourself is a key move, not a field edit. It copies to the new key, merges by comment id if a store already exists there, then deletes the source; a refused write returns `false` and commits nothing, so the reviewer keeps their comments and that export simply goes out unattributed.
...
- **Rename:** `renameReviewer()` moves a corpus between reviewer keys (copy → merge-by-id → delete). Copy-then-delete is deliberate: a quota-refused write must not destroy the source.
