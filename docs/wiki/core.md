...
Armed-mode hover outline: while annotate mode is armed, `_onHoverMove` — a capture-phase passive `pointermove` listener attached in `_enterAnnotateMode` and detached in `_exitAnnotateMode` (zero move-listener work at rest, mirroring the gesture controller's press-scoped pattern) — rAF-throttles `_paintHover()`, which boxes the element under the cursor with a non-interactive `.hl` outline (accent border + faint accent wash, `pointer-events:none`) rendered inside the shadow root from `getBoundingClientRect` — host element styles/classes are never touched. Pinflow's own UI is skipped via the same composedPath guard as `_onDocumentClick` (crossing onto pinflow chrome hides the box), and `_clearHover()` removes the outline and cancels any pending frame on disarm/pin placement/Escape.
...
**`ui/styles.ts`** — Inline CSS (hand-minified) for the shadow tree. Defines `.root`, `.control` (bottom-right button), `.pin` (numbered badge), `.hl` (armed-mode hover outline; transition dropped under `prefers-reduced-motion`), `.panel` (info drawer), `.input` (text editor popup), `.drawer` (builder-mode checkbox list). Theme tokens are CSS variables (`--pf-*`) consumed with fallbacks; media queries handle dark mode and mobile.
