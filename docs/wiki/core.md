...
**`ui/styles.ts`** — Inline CSS (hand-minified) for the shadow tree. Defines `.root`, `.control` (bottom-right arm/disarm pill), `.pin` (numbered badge), `.hl` (armed-mode hover outline; `[data-marquee]` adds the page-dimming box-shadow; transition dropped under `prefers-reduced-motion`), `.panel` (export sheet / confirmation), `.input` (text editor popup), `.drawer` (builder-mode checkbox list). Theme tokens are CSS variables (`--pf-*`) consumed with fallbacks; media queries handle dark mode and mobile.
