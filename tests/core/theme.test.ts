...
describe('theme tokens (A1)', () => {
  ...
  it('stylesheet consumes the tokens via var() with stock fallbacks', () => {
    ...
    // Scheme-aware stock fallback (0.11.0): the armed clear is a standing
  ...
  it('every typable field is 16px on coarse pointers — iOS Safari must not auto-zoom on focus', () => {
    ...
    // The block also carries the 44px panel-button floor (0.11.0), so the
    ...
    // media query fails the assertion (0.11.0 review #4).
