...
| `anchor`     | `Anchor`                          | Selectors + position + element context (name/role/heading, computed-style snapshot, image src). `anchor.layer` (0.12.0, additive) binds a pin taken inside a modal to that dialog by accessible name; it resolves only inside an open dialog of that name and parks otherwise. |
