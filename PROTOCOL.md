...
| `anchor`     | `Anchor`                          | Selectors + position + element context (name/role/heading, computed-style snapshot, image src). `selectors.role` / `selectors.name` (0.12.0, additive) carry the role-and-accessible-name locator rung, present only when a name exists. `anchor.layer` (0.12.0, additive) binds a pin taken inside a modal to that dialog by accessible name; it resolves only inside an open dialog of that name and parks otherwise. |
